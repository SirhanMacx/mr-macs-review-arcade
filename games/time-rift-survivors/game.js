(() => {
  ...
  function initSetupExtras() {
    _trsRenderResumeCard();
    _trsRenderLeaderboardPanel();
  }
  function _trsRenderResumeCard() {
    const card = document.getElementById("resumeCard");
    if (!card) return;
    if (!window.MrMacsSessions) { card.hidden = true; return; }
    let prev = null;
    try { prev = window.MrMacsSessions.load("time-rift-survivors"); } catch (e) {}
    if (!prev || !prev.state || !prev.ts) { card.hidden = true; return; }
    if (Date.now() - prev.ts > 24 * 3600 * 1000) { card.hidden = true; return; }
    const s = prev.state || {};
    const totalSec = Math.floor(s.runTime || 0);
    ...
    card.innerHTML =
      '<div class="resume-card-head">' +
        ...
        '<span class="resume-card-time">' + _trsSafe(_trsFmtAgo(prev.ts)) + '</span>' +
      '</div>' +
      '<div class="resume-card-meta">' + _trsSafe(minLabel) + ' survived · level ' + (s.level || 1) + '</div>' +
    ...
    if (resumeBtn) {
      resumeBtn.addEventListener("click", () => {
        // Survivors-style runs reset on death; start a fresh run with toast
        // acknowledgement of the prior session.
        try {
          if (window.MrMacsToast) window.MrMacsToast.push({
            icon: "⏯", title: "Resuming · last run " + minLabel,
            sub: "Reached level " + (s.level || 1), tone: "info", ms: 3500
          });
        } catch (e) {}
    ...
    if (freshBtn) {
      freshBtn.addEventListener("click", () => {
        try { window.MrMacsSessions.clear("time-rift-survivors"); } catch (e) {}
        card.hidden = true;
      });
    }
  }
  function _trsRenderLeaderboardPanel() {
    const panel = document.getElementById("leaderboardPanel");
    if (!panel) return;
    if (!window.MrMacsLeaderboards) { panel.hidden = true; return; }
    let rows = [];
    try { rows = window.MrMacsLeaderboards.top("time-rift-survivors", 5) || []; } catch (e) { rows = []; }
    panel.hidden = false;
    if (!rows.length) {
      panel.innerHTML =
    ...
    panel.innerHTML =
      ...
      rows.map((r, i) =>
        '<li class="lb-row">' +
          ...
          '<span class="lb-avatar">' + _trsSafe(r.avatar || "") + '</span>' +
          '<span class="lb-name">' + _trsSafe(r.name || "Trainer") + '</span>' +
          '<span class="lb-score">' + Math.round(r.score || 0).toLocaleString() + '</span>' +
          '<span class="lb-ago">' + _trsSafe(_trsFmtAgo(r.ts || 0)) + '</span>' +
        '</li>'
      ).join("") +
      '</ol>';
  }
